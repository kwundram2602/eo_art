...
def _detect_kind(ds: xr.Dataset) -> Literal["raster", "timeseries", "dem"]:
    ...
    if "time" in first_var.dims:
        return "timeseries"
    # Single float band without a time dim → treat as DEM.
    if first_var.sizes.get("band", 0) == 1 and np.issubdtype(
        first_var.dtype, np.floating
    ):
        return "dem"
    return "raster"
...
class EOData:
    ...
    def scene3d(self, dem: EOData | None = None) -> Scene3D:
        # Pass self as dem when called on a DEM; use .drape() to add texture.
        from ..render3d.scene import Scene3D
